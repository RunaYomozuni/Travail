...
function numbrePositif(numbers) {
    let table = [];
    for (let i = 0; i < numbers.length; i++){
        if (numbers[i] > 0){
            table.push(numbers[i]);
        }
    }
    return table;
}
numbrePositif([-1,2,-3,4]);
...
function special(regex,text) {
    return regex.test(text);
}
special(/\W/,'jonascompper2@gmail.com!');
